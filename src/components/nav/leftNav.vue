<template>
  ...
  <aside :class="{showSidebar:!collapsed}">
    ...
    <!--导航菜单-->
    <el-menu :default-active="$route.path" router :collapse="collapsed" ref="leftNavigation" class="menu-color">
      <template v-for="(issue,index) in $router.options.routes">
        ...
        <template v-if="issue.name === $store.state.leftNavState">
          <template v-for="(item,index) in issue.children">
            ...
            <el-menu-item v-else-if="item.leaf" :index="item.path" :key="item.path"
